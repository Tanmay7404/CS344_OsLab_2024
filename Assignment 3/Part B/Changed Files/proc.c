...
static struct inode*
proc_create_file(char *path, short type, short major, short minor)
{
  struct inode *ip, *dp;
  char name[DIRSIZ];
...
struct proc* rcqop2(){
	...
	struct proc* p=swap_in_queue.queue[swap_in_queue.s];
	(swap_in_queue.s)++;
	(swap_in_queue.s)%=NPROC;
	release(&swap_in_queue.lock);
	return p;
}
...
int cqpush2(struct proc* p){
	acquire(&swap_in_queue.lock);
	if((swap_in_queue.e+1)%NPROC==swap_in_queue.s){
		release(&swap_in_queue.lock);
		return 0;
	}
	swap_in_queue.queue[swap_in_queue.e]=p;
	(swap_in_queue.e)++;
	(swap_in_queue.e)%=NPROC;
  ...
  while(swap_out_queue.s!=swap_out_queue.e){
    ...
    for(int i=0;i<NPDENTRIES;i++){
      ...
      for(int j=0;j<NPTENTRIES;j++){
        ...
        //file name
        char c[50];
        string_from_int(pid,c);
        int x=strlen(c);
        c[x]='_';
        string_from_int(virt,c+x+1);
        safestrcpy(c+strlen(c),".swp",5);

        // file management
        int fd=proc_open_file(c, O_CREATE | O_RDWR);
        if(fd<0){
          cprintf("error creating or opening file: %s\n", c);
          panic("swap_out_process");
        }

        if(proc_write_file(fd,(char *)pte, PGSIZE) != PGSIZE){
...
void swap_in_process_function(){
	...
	while(swap_in_queue.s!=swap_in_queue.e){
	    ...
	    int fd=proc_open_file(c,O_RDONLY);
	    if(fd<0){
	    	release(&swap_in_queue.lock);
